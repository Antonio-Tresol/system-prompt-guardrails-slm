...
class TestEndToEndPipeline:
    ...
    def test_full_pipeline_with_markdown(self, tmp_path: Path) -> None:
        ...
        # Assert: Privacy detection ran (may default to public if API connection fails)
        privacy_levels = [pr.privacy_level for pr in result["privacy_results"]]
        # Note: Connection errors cause fallback to 'public', which is acceptable behavior
        assert all(level in ["public", "mixed", "private"] for level in privacy_levels), (
            "Privacy levels should be valid"
        )
    ...
    def test_full_pipeline_with_pdf(self, tmp_path: Path) -> None:
        """Test complete pipeline processes PDF documents with OCR.

        This test validates:
        - PDF document loading via Docling
        - OCR extraction of text content
        - PDF chunking with page metadata
        - Privacy detection on PDF content
        - Vector store insertion and querying
        """
        import urllib.request

        # Arrange: Download test PDF from arXiv (Attention is All You Need paper)
        pdf_url = "https://arxiv.org/pdf/1706.03762"
        test_pdf = tmp_path / "test_paper.pdf"

        # Validate URL scheme for security
        if not pdf_url.startswith(("http:", "https:")):
            pytest.skip("Invalid URL scheme")

        try:
            urllib.request.urlretrieve(pdf_url, test_pdf)  # noqa: S310
        ...
        try:
            ...
            # Assert: Chunks were created
            assert len(result["chunks"]) > 0, "No chunks were created from PDF"
            assert len(result["privacy_results"]) > 0, "No privacy results generated"
            assert len(result["metadatas"]) > 0, "No metadata extracted"

            # Assert: Metadata is properly populated for PDF
            for metadata in result["metadatas"]:
                assert metadata.num_tokens > 0, "Token count should be positive"
                assert metadata.num_words > 0, "Word count should be positive"
                assert metadata.source_file.endswith("test_paper.pdf")
            ...
            print(f"   - Processed {len(result['chunks'])} chunks from PDF")
            print(f"   - Privacy levels found: {privacy_levels}")
            print(f"   - Total chunks in DB: {total_chunks}")
            print(f"   - Query returned {len(query_results)} results")

        finally:
            # Cleanup: Delete the downloaded PDF
            if test_pdf.exists():
                test_pdf.unlink()
